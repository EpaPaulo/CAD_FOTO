...
export type TraceStep = 'corners' | 'trace' | 'edit'

type Step = TraceStep

/** Where a finished trace leads, so the caller can navigate or stay in place. */
...
interface Props {
  ...
  /** Reported once the tracer list loads, so the caller can label its steps. */
  onTracerCountChange?: (count: number) => void
  onFinished: (result: TraceFinished) => void
}

/** The steps shown for a trace, which gain an outline step for extra tracers. */
export function traceStepLabels(tracerCount: number) {
  return tracerCount <= 1
    ? ['Check photo', 'Check tools', 'Design & download']
    : ['Check photo', 'Find outlines', 'Check tools', 'Design & download']
}

export function traceStepIndex(step: Step, tracerCount: number) {
  if (tracerCount <= 1) return step === 'corners' ? 0 : 1
  return step === 'corners' ? 0 : step === 'trace' ? 1 : 2
}
...
export function TraceWorkspace({ sessionId, step, onStepChange: setStep, onTracerCountChange, onFinished }: Props) {
  ...
  useEffect(() => {
    ...
    load()
  }, [sessionId, setIncludedPolygons, setStep])
  ...
  useEffect(() => {
    onTracerCountChange?.(tracers.length)
  }, [tracers.length, onTracerCountChange])
  ...
  async function handleFinish(destination: 'bin' | 'library') {
    ...
    try {
      ...
      polygonsDirtyRef.current = false
      onFinished({ destination, href, binId: completionRef.current.bin?.id })
    } catch (err) {
